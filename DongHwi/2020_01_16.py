...
# 컨투어 x,y 최소, 최대 값 찾기
def findMinMaxPoint(bi_image, image, contour,hierarchy):
    min_max_list = []
        ...
        if (x_max - x_min) <= 10 and (y_max - y_min) <= 10:
            ...
            * 글자 ( 자음, 모음 ) 영역을 그렸을때 픽셀양이 45% 이하인걸 확인.
            ...
            if (pixel_sum * 100 <= 55 and pixel_sum * 100 >= 10) or pixel_sum * 100 > 95:
                ...
                #중복을 제거해주는 부분
                a = list(set([tuple(set(ll[0])) for ll[0] in ll[0]]))
                if len(a) <= 30:
                    op_draw = cv2.drawContours(image,contour,i,color=(0,0,255),thickness=1)
...
if __name__ == '__main__':
    ...
    for bi_image in result_filter_list:
        ...
        point_list = findMinMaxPoint(bi_image, img_color, contour,hierarchy)
