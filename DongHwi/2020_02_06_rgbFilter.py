...
def blackImageDraw(binding_image, black_image_list, color_list):
    ...
    for i in range(h):
        for j in range(w):
            b, g, r = binding_image[i][j]

            for index, color in enumerate(color_list):
                if (b, g, r) == color:
                    black_image_list[index][i][j] = (255,255,255)
                    break
...
# 컨투어 찾기
def findContour(black_draw_image_list):
...
if __name__ == '__main__':
    ...
    bgr_image = cv2.imread('image/test_image/3.jpg')
    ...
    blackImageDraw(bgr_binding_image, black_draw_image_list, color_list)

    contour_list, hierarchy_list = findContour(black_draw_image_list)
