...
#특정 폴더에 있는 이미지의 이름을 전부 가져와서 2차원 배열로만듦
#[파일이름,단어]  ex ['ga.png','ga']
path='default/'
...
for i in file_List:
    value={}
    value['file_Name']=i
    value['value']=i.split('.')[0]
    standardFile_List[len(standardFile_List)]=value
# print("처음",standardFile_List)

comparisonFile = 'a/image1.jpg'
...
def pixelInfo(image, pixelPoint):
    ...
    for i in range(len(pixelPoint)):
        ...
        imgCut = image[pixelPoint[i][1]:pixelPoint[i][1] + pixelPoint[i][3],
                 pixelPoint[i][0]:pixelPoint[i][0] + pixelPoint[i][2]]
        ...
        for j in range(divisionCount):
            for k in range(divisionCount):
                ...
                contourCut = imgResize[k * pixelPoint[i][3]:k * pixelPoint[i][3] + pixelPoint[i][3],
                             j * pixelPoint[i][2]:j * pixelPoint[i][2] + pixelPoint[i][2]]
...
def standard_image_to_gray(path,FileList):

    for u in FileList.keys():

        # 기준 이미지, 비교 이미지 설정
        imgStandard = cv.imread(path+FileList[u]['file_Name'], cv.IMREAD_COLOR)
        ...
        pixelinfo=pixelInfo(imgBinary,boxPoint)

        add = {}
        add['boxPoint'] = boxPoint
        add['contoursDistance'] = contoursDistance
        add['pixelInfo']=pixelinfo
        FileList[u].update(add)

    return FileList
def comparison_image_to_gray(comparisonFile):
    ########################################################################################
    ...
    contoursComparison, hierarchyComparison = cv.findContours(imgBinaryComparison, cv.RETR_EXTERNAL,
    ...
    return boxPointComparison,comparisonPixelInfo
...
standardFile_List=standard_image_to_gray(path,standardFile_List)
# print(standardFile_List)
boxPointComparison,comparisonPixelInfo=comparison_image_to_gray(comparisonFile)


# print("qweqwe",comparisonPixelInfo)
...
for i in range(len(boxPointComparison)):
    ...
    #여기부터 반복문
    for u in standardFile_List.keys():
        # imgComparison = cv.imread(comparisonFile, cv.IMREAD_COLOR)
        ratio = boxPointComparison[i][2] / standardFile_List[u]['boxPoint'][0][2]
        ...
        if boxPointComparison[i][2] > exceptionPixel and boxPointComparison[i][3] > exceptionPixel:
            ...
            if (boxPointComparison[i][3] <= (standardFile_List[u]['boxPoint'][0][3] * ratio) + errorRangePixel and boxPointComparison[i][3] >= (
                    standardFile_List[u]['boxPoint'][0][3] * ratio) - errorRangePixel):
                ...
                ########################################################################################
                flag = []
                for j in range(len(standardFile_List[u]['contoursDistance'])):
                    # 다음 영역이 있어야 하는 예상 위치
                    expectX = boxPointComparison[i][0] + (standardFile_List[u]['contoursDistance'][j][0] * ratio)
                    expectY = boxPointComparison[i][1] + (standardFile_List[u]['contoursDistance'][j][1] * ratio)
                    # 정말로 예상 위치에 다음 영역이 존재하는지 확인한다.
                    for k in range(len(boxPointComparison)):
                        # 예상 위치에 정말 영역이 존재한다면 몇번째 영역인지 저장한다.
                        if (boxPointComparison[k][0] >= expectX - errorRangePixel and boxPointComparison[k][
                            0] <= expectX + errorRangePixel and boxPointComparison[k][1] >= expectY - errorRangePixel and
                                boxPointComparison[k][1] <= expectY + errorRangePixel):
                ...
                ########################################################################################
                finalOrder = []
                if len(standardFile_List[u]['contoursDistance']) == len(flag):
                    finalOrder.append(i)
                    ...
                    for m in range(len(finalOrder)):
                        for n in range(0, pow(divisionCount, 2)):
                            ...
                            minmaxRange = math.sqrt(pow(standardFile_List[u]['pixelInfo'][m][n] - comparisonPixelInfo[finalOrder[m]][n], 2))
                    ...
                    if maxRange <= errorEqualRate:
                        ...
                        print(standardFile_List[u]['value'])
