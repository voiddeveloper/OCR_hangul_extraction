...
if __name__ == '__main__':
    ...
    # rgb 이미지 불러오기
    img_color = cv2.imread('hwang/imgSet/test1_1.png')
    ...
    #################################################################
    # 이 영역만 테스트로 수정중
    contour = findContour(black_filter_result_image)
    cv2.imshow('test', black_filter_result_image)
    imgCopy = black_filter_result_image.copy()
    imgCopyColor = cv2.cvtColor(imgCopy, cv2.COLOR_GRAY2BGR)

    for cnt in contour:
        # cv2.drawContours(imgCopyColor, [cnt], 0, (255, 0, 0), 1)
        epsilon = 0.008 * cv2.arcLength(cnt, True)
        approx = cv2.approxPolyDP(cnt, epsilon, True)
        print("approx : ", len(approx))

        # if len(approx) >= 4 and len(approx) <= 12:
        cv2.drawContours(imgCopyColor, [approx], 0, (0, 0, 255), 1)

    cv2.imshow('test2', imgCopyColor)
    cv2.waitKey(0)
