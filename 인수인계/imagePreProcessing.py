...
if __name__ == '__main__':
    ...
    try:
        basic_image = Image.open("C:\ocr_image\image8/comparison_image_9.png")
        print('jpg이미지 선택')
    except:
    ...
    # cv2.imwrite('sdf.png',closing_image)
    # cv2.waitKey(0)
    ...
    # image, contours, hierachy = cv2.findContours(closing_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    contours, hierachy = cv2.findContours(closing_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    # image, contours, hierachy = cv2.findContours(closing_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    try:
        for i in range(len(contours)):
            ...
            if x_distance == 0 or y_distance == 0 or \
                    (x_distance / y_distance) < 0.4 or \
                ...
                # 왼쪽하단, 오른쪽 상단
                point_list.append([X_MAX, Y_MIN, X_MIN, Y_MAX])
                ...
                # draw_image = cv2.circle(original_image, (X_MIN, Y_MIN), 4, (0, 255, 255), -1)
                # draw_image = cv2.circle(original_image, (X_MAX, Y_MAX), 4, (0, 255, 255), -1)
                # draw_image = cv2.circle(original_image, (X_MAX, Y_MIN), 4, (0, 255, 255), -1)
                # draw_image = cv2.circle(original_image, (X_MIN, Y_MAX), 4, (0, 255, 255), -1)
    except:
        print('글자찾기 에러')
    ...
    for cnt in contours:
        ...
        # print(box[2])
        # print(box[3])
        # rotate_image = cv2.circle(original_image, (box[0][0], box[0][1]), 1, (0, 0, 255), 7)
        # rotate_image = cv2.circle(original_image, (box[1][0], box[1][1]), 1, (0, 0, 255), 7)
        # rotate_image = cv2.circle(original_image, (box[2][0], box[2][1]), 1, (0, 0, 255), 7)
        # rotate_image = cv2.circle(original_image, (box[3][0], box[3][1]), 1, (0, 0, 255), 7)
        # rotate_image = cv2.drawContours(original_image, [box], 0, (0,255,0), 2)

    # cv2.imwrite('sdf.png',rotate_image)
    ...
    cv2.imshow('result_image', original_image)
    cv2.waitKey(0)

    # 그려진 이미지 저장
    # cv2.imwrite('../rec_result.jpg', original_image)
