...
1번 
"""
#기준 픽셀의 색과 다른 색을 가진 픽셀을 모두 저장하는 변수
width_pixels = []
height_pixels = []
...
for l in range(y):
    ...
    for f in range(x):
        if f <= x - 2 and l <= y - 2:
            # b g r = 현재 기준 픽셀의 색상값
            # right_pixel_b g r = 현재 기준 픽셀의 오른쪽에 있는 픽셀 색상값
            # down_pixel_b g r = 현재 기준 픽셀의 아래에 있는 픽셀 생상값
            b, g, r = image.getpixel((f, l))
            right_pixel_b, right_pixel_g, right_pixel_r = image.getpixel((f, l + 1))
            down_pixel_b, down_pixel_g, down_pixel_r = image.getpixel((f + 1, l))

            right_range = math.sqrt((b - right_pixel_b) ** 2 + (g - right_pixel_g) ** 2 + (r - right_pixel_r) ** 2)
            down_range = math.sqrt((b - down_pixel_b) ** 2 + (g - down_pixel_g) ** 2 + (r - down_pixel_r) ** 2)

            if right_range > 30:
                width_pixels.append([f, l])
            if down_range > 30:
                height_pixels.append([f, l])

width_pixels.sort(key=lambda x: x[1])
height_pixels.sort(key=lambda x: x[0])
        ...
        for j in width_line.keys():
            ...
            for k, v in enumerate(width_line[j]):
                ...
                if (v[0] + 1 == i[0] and v[1] == i[1]) or (v[0] + 2 == i[0] and v[1] == i[1]) \
                    ...
                    #pixel_list 한개의 가로선인 픽셀의 리스트
                    pixel_list = list(width_line[j])
                    pixel_list.append([i[0], i[1]])
                    width_line[j] = pixel_list
                    try:
...
# height_line = 세로선들의 dictionary
height_line = {}
for i in height_pixels:
    # r의 길이가 0이 아니라면 비교
        ...
        for j in height_line.keys():
            ...
            for k, v in enumerate(height_line[j]):
                if (v[1] + 1 == i[1] and v[0] == i[0]) or (v[1] + 2 == i[1] and v[0] == i[0]) \
                    ...
                    #pixel_list 한개의 세로선인 픽셀의 리스트
                    pixel_list = list(height_line[j])
                    pixel_list.append([i[0], i[1]])
                    height_line[j] = pixel_list
                    try:
                        height_pixels.remove(i)
                    except ValueError:
                        pass
                    break
...
"""
3번 
    가로선에 세로의 끝점들이 포함되어있다면
    가로선을 2개로 나눠서 저장하는 부분을 따로 만들었다
    이유)
    (가로선을 1개로만 판단하다보니 겹쳐져있는 네모는 나오지않음)
"""

#가로선과 세로선들을 정렬하고 그다음 저장되는 리스트
width_list = []
height_list = []
    ...
    for s_, j in enumerate(height_line[i]):
        for k in width_line.keys():
            for r_, h in enumerate(width_line[k]):
                ...
                if height_line[i][s_][0] - 3 <= width_line[k][r_][0] and height_line[i][s_][0] + 3 >= width_line[k][r_][
                    0] and height_line[i][s_][1] - 3 <= width_line[k][r_][1] and height_line[i][s_][1] + 3 >= \
                        width_line[k][r_][1]:
...
def tuples(pixel_list):
    try:
        return tuple(tuples(a) for a in pixel_list)
    except TypeError:
        return pixel_list
...
for i in width_list:
    ...
    width_line[len(width_line)] = result_list
for i in height_list:
    result_list = []
    for j in i:
        result_list.append([j[0], j[1]])
    height_line[len(height_line)] = result_list



# 새로운 가로,세로선을 추가하고 중복제거
over_width = []
over_height = []
for i in width_line.keys():
    over_width.append(width_line[i])
for i in height_line.keys():
    over_height.append(height_line[i])
over_height = set(tuples(over_height))
over_width = set(tuples(over_width))
width_line = {}
height_line = {}
for i, v in enumerate(over_width):
    width_line[i] = v
for i, v in enumerate(over_height):
    height_line[i] = v
    ...
    for r_, j in enumerate(width_line[i]):
        for k in height_line.keys():
            for s_, h in enumerate(height_line[k]):
                ...
                if width_line[i][r_][0] - 3 <= height_line[k][s_][0] and width_line[i][r_][0] + 3 >= height_line[k][s_][
                    0] and width_line[i][r_][1] - 3 <= height_line[k][s_][1] and width_line[i][r_][1] + 3 >= \
                        height_line[k][s_][1]:
...
"""
4번
    위에서 만들어놓은 가로선들과 세로선들을 가지고 네모를 찾는다
...
for i in width_line.keys():
    ...
    for j in height_line.keys():
        ...
        if abs(width_line[i][0][0] - height_line[j][0][0]) <= 3 and abs(
                width_line[i][0][1] - height_line[j][0][1]) <= 3:
            for k in height_line.keys():
                if eq(j, k):
                    # 같은 리스트인거 넘기기
                    continue
                    ...
                    # 다시 세로 리스트를 반복
                    if abs(width_line[i][-1][0] - height_line[k][0][0]) <= 3 and abs(
                            width_line[i][-1][1] - height_line[k][0][1]) <= 3:
                        # 오른쪽 세로 까지 맞다면 이제 가로를 다시 비교
                        for q in width_line.keys():
                            if eq(i, k):
                                continue
                            else:
                                ...
                                # s[jj] 오른쪽 세로
                                # print(r[ii])
                                if (abs(width_line[q][0][0] - height_line[j][-1][0]) <= 3 and abs(
                                        width_line[q][0][1] - height_line[j][-1][1]) <= 3) and (
                                        abs(width_line[q][-1][0] - height_line[k][-1][0]) <= 3 and abs(
                                    width_line[q][-1][1] - height_line[k][-1][1]) <= 3):
                                    # print("ㅋ")
                                    last.append([height_line[j][0][0], height_line[j][0][1], height_line[k][-1][0],
                                                 height_line[k][-1][1]])
...
for i in last:
    if len(result) != 0:
        for v, j in enumerate(result):
            #오차값이 10이다
            if ((j[0] - 10 <= i[0] and j[0] + 10 >= i[0]) and (j[1] - 10 <= i[1] and j[1] + 10 >= i[1])) and (
                    (j[2] - 10 <= i[2] and j[2] + 10 >= i[2]) and (j[3] - 10 <= i[3] and j[3] + 10 >= i[3])):
                break
            if v == len(result) - 1:
                result.append(i)
    else:
